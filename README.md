...
1. Cloned the website source code from GitHub using git clone
2. Created a new S3 bucket using the AWS CLI (aws s3 mb)
3. Uploaded all website files to the bucket using aws s3 sync
4. Enabled static website hosting on the bucket (aws s3 website)
5. Configured a bucket policy to allow public read access (aws s3api put-bucket-policy)
6. Verified the live deployment by accessing the public S3 website endpoint
